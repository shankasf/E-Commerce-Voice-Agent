...
public class ClearUserTempTool : ITool
{
    public string Name => "clear_user_temp";
    ...
    public async Task<ToolResult> ExecuteAsync(Dictionary<string, object> arguments)
    {
        ...
        try
        {
            ...
            {
                Success = true,
                Output = $"Cleared {deletedFiles} files ({deletedSize / (1024.0 * 1024.0):F2} MB) from User Temp",
