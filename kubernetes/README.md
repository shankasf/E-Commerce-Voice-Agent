...
./k8s-manager.sh build all

# 3. Deploy all applications
./k8s-manager.sh deploy-all

# 4. Check status
./k8s-manager.sh status
```
...
├── k8s-manager.sh           # Main management script
...
# Build Docker images
./k8s-manager.sh build <app|all>

# Deploy applications
./k8s-manager.sh deploy <app>
./k8s-manager.sh deploy-all

# Delete applications
./k8s-manager.sh delete <app>
./k8s-manager.sh delete-all

# Check status
./k8s-manager.sh status

# Check ports
./k8s-manager.sh ports

# View logs
./k8s-manager.sh logs <app>

# Scale application
./k8s-manager.sh scale <app> <replicas>

# Restart application
./k8s-manager.sh restart <app>
```
...
6. **Update k8s-manager.sh** with build/deploy commands
7. **Deploy**: `./k8s-manager.sh deploy <app-name>`
