...
// PostgREST-style operators (used in parseFilters regex and buildWhereClause switch)
// eq: '=', neq: '!=', gt: '>', gte: '>=', lt: '<', lte: '<=',
// like: 'LIKE', ilike: 'ILIKE', is: 'IS', in: 'IN', fts: '@@'
...
function parseFilters(query: Record<string, unknown>): Filter[] {
  ...
  for (const [key, value] of Object.entries(query)) {
    if (reservedParams.includes(key) || typeof value !== 'string') continue;
...
function parseOrder(orderParam: string | undefined): string {
  ...
  const orders = orderParam.split(',').map((o) => {
    ...
    if (!isSafeIdentifier(col)) return null;
...
function parseSelect(selectParam: string | undefined, allColumns: string[]): string {
...
router.get(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const orderClause = parseOrder(query.order as string | undefined);
    const selectClause = parseSelect(query.select as string | undefined, allColumns);

    const limit = Math.min(Number.parseInt(query.limit as string || '100', 10), 1000);
    const offset = Number.parseInt(query.offset as string || '0', 10);
...
router.get(
  '/:dbName/:schema/:table/:id',
  requireAuth,
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const { dbName, schema, table, id } = req.params;
    const primaryKey = (req.query.pk as string) || 'id';
    ...
    const result = await dbPool.query(
      `SELECT * FROM ${quoteIdent(schema)}.${quoteIdent(table)} WHERE ${quoteIdent(primaryKey)} = $1`,
      [id]
    );
