...
router.get(
  '/:dbName/tables/:schema/:table',
  requireAuth,
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const { dbName, schema, table } = req.params;
    const limit = Math.min(Number.parseInt(req.query.limit as string || '50', 10), 500);
    ...
    const columnsResult = await dbPool.query<ColumnInfo>(
    ...
    // Get indexes
    const indexesResult = await dbPool.query<IndexInfo>(
    ...
    // Get constraints
    const constraintsResult = await dbPool.query<ConstraintInfo>(
...
router.get(
  '/:dbName/tables/:schema/:table/export/csv',
  requireAuth,
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const { dbName, schema, table } = req.params;
...
router.post(
  '/:dbName/tables/:schema/:table/truncate',
  requireAuth,
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const { dbName, schema, table } = req.params;
...
router.post(
  '/:dbName/tables/:schema/:table/columns',
  requireAuth,
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const { dbName, schema, table } = req.params;
...
router.delete(
  '/:dbName/tables/:schema/:table/columns/:column',
  requireAuth,
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
...
router.post(
  '/:dbName/tables/:schema/:table/rows',
  requireAuth,
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const { dbName, schema, table } = req.params;
    ...
    const dbPool = getPoolForDb(config.pg, dbName);
    const result = await dbPool.query(
      `INSERT INTO ${quoteIdent(schema)}.${quoteIdent(table)} (${columns.join(', ')}) VALUES (${placeholders.join(', ')}) RETURNING *;`,
      values
    );
...
router.patch(
  '/:dbName/tables/:schema/:table/rows/:id',
  requireAuth,
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const dbPool = getPoolForDb(config.pg, dbName);
    const result = await dbPool.query(
      `UPDATE ${quoteIdent(schema)}.${quoteIdent(table)} SET ${setClauses.join(', ')} WHERE ${quoteIdent(primaryKey)} = $${values.length} RETURNING *;`,
      values
    );
...
router.delete(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const { dbName, schema, table, id } = req.params;
    const primaryKey = (req.query.primaryKey as string) || 'id';
    ...
    const dbPool = getPoolForDb(config.pg, dbName);
    const result = await dbPool.query(
      `DELETE FROM ${quoteIdent(schema)}.${quoteIdent(table)} WHERE ${quoteIdent(primaryKey)} = $1 RETURNING *;`,
      [id]
    );
...
router.post(
  '/:dbName/tables/:schema/:table/indexes',
  requireAuth,
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const { dbName, schema, table } = req.params;
    ...
    const columnList = columns.split(',').map((c: string) => c.trim()).filter(isSafeIdentifier);
    ...
    await dbPool.query(
      `CREATE ${uniqueClause}INDEX ${quoteIdent(name)} ON ${quoteIdent(schema)}.${quoteIdent(table)} (${columnList.map(quoteIdent).join(', ')});`
    );
...
router.delete(
  '/:dbName/tables/:schema/:table/indexes/:index',
  requireAuth,
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
