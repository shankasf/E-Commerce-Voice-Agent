...
router.get('/', requireAuth, asyncHandler(async (_req, res) => {
    ...
    const uptimeResult = await adminPool.query(`SELECT date_trunc('second', current_timestamp - pg_postmaster_start_time()) as uptime;`);
    const connectionsResult = await adminPool.query(`SELECT count(*) as active FROM pg_stat_activity WHERE state = 'active';`);
    const maxConnResult = await adminPool.query('SHOW max_connections;');
    const totalSizeResult = await adminPool.query(`SELECT SUM(pg_database_size(datname)) as total FROM pg_database WHERE datistemplate = false;`);
    const dbCountResult = await adminPool.query(`SELECT count(*) as count FROM pg_database WHERE datistemplate = false;`);
    // Active connections
    const activeConnsResult = await adminPool.query(`SELECT pid, datname, usename, client_addr::text, state, query,
       ...
       LIMIT 50;`);
    // Database sizes
    const dbSizesResult = await adminPool.query(`SELECT d.datname, pg_database_size(d.datname) as size_bytes,
              (SELECT count(*) FROM pg_stat_activity WHERE datname = d.datname)::int as numbackends
       FROM pg_database d
       WHERE datistemplate = false
       ORDER BY pg_database_size(d.datname) DESC;`);
    // Locks
    const locksResult = await adminPool.query(`SELECT l.pid, d.datname, c.relname, l.locktype, l.mode, l.granted
       FROM pg_locks l
    ...
    let slowQueries = [];
    try {
        const slowResult = await adminPool.query(`SELECT query, calls::int, total_exec_time as total_time,
    ...
    let replication = [];
    try {
        const repResult = await adminPool.query(`SELECT client_addr::text, state, sent_lsn::text, write_lsn::text, replay_lag::text
...
router.post('/connections/:pid/kill', requireAuth, asyncHandler(async (req, res) => {
...
router.post('/databases/:dbName/vacuum', requireAuth, asyncHandler(async (req, res) => {
...
router.post('/databases/:dbName/analyze', requireAuth, asyncHandler(async (req, res) => {
    const { dbName } = req.params;
    if (!isSafeIdentifier(dbName)) {
        throw new ValidationError('Invalid database name');
    }
