...
router.get('/', requireAuth, asyncHandler(async (_req, res) => {
    const result = await adminPool.query(`SELECT datname,
...
router.get('/:dbName', requireAuth, asyncHandler(async (req, res) => {
    const { dbName } = req.params;
    if (!isSafeIdentifier(dbName)) {
        throw new ValidationError('Invalid database name');
    }
    // Get database info
    const dbResult = await adminPool.query(`SELECT datname,
              pg_database_size(datname) AS size_bytes,
              pg_get_userbyid(datdba) AS owner,
              datallowconn
       FROM pg_database
    ...
    const tablesResult = await dbPool.query(`SELECT table_schema, table_name
...
router.delete('/:dbName', requireAuth, asyncHandler(async (req, res) => {
    const { dbName } = req.params;
...
router.get('/:dbName/schemas', requireAuth, asyncHandler(async (req, res) => {
    const { dbName } = req.params;
    if (!isSafeIdentifier(dbName)) {
        throw new ValidationError('Invalid database name');
    }
    const dbPool = getPoolForDb(config.pg, dbName);
    const result = await dbPool.query(`SELECT schema_name
...
router.get('/:dbName/roles', requireAuth, asyncHandler(async (_req, res) => {
