...
import { Phone, Clock, CheckCircle, XCircle, AlertCircle, RefreshCw } from 'lucide-react';
...
export function CallsPage() {
    ...
    useEffect(() => {
        ...
        const handleDashboardUpdate = (data: unknown) => {
            ...
            if (update.type === 'call') {
                // Refetch data when call events occur
        ...
        const handleCallUpdate = () => {
            // Refetch on any call update
            queryClient.invalidateQueries({ queryKey: ['dashboard-calls'] });
            setLastUpdate(new Date());
        };

        const handleCallEnd = () => {
            // Refetch when calls end
            queryClient.invalidateQueries({ queryKey: ['dashboard-calls'] });
            setLastUpdate(new Date());
        };
